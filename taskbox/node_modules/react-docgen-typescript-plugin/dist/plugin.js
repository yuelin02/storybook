...
const generateDocgenCodeBlock_1 = tslib_1.__importDefault(require("react-docgen-typescript-loader/dist/generateDocgenCodeBlock"));
...
function processModule(parser, webpackModule, tsProgram, loaderOptions) {
    ...
    const docs = generateDocgenCodeBlock_1.default(Object.assign({ filename: webpackModule.userRequest, source: webpackModule.userRequest, componentDocs }, loaderOptions)).substring(webpackModule.userRequest.length);
...
const matchGlob = (globs) => (filename) => Boolean(filename && globs.find((g) => micromatch_1.default([filename], g).length));
...
class DocgenPlugin {
    ...
    apply(compiler) {
        ...
        const parser = (tsconfigPath && docGen.withCustomConfig(tsconfigPath, docgenOptions)) ||
